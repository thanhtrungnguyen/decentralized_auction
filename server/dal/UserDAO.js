...
const getUserById = async (userId) => {
    ...
    var user = null;
    if (role === 'MANAGER') {
        await conn.sobject('User__c').find({ Id: userId }, (err, result) => {
            if (err) console.err(err)
            user = result
        })
    }
    else {
        await conn.sobject('Contact__c').find({User_Id__c: userId},(err,result)=>{
            if (err) console.err(err)
            user = result
        })
    }
...
const getUserRole = async (userId) => {
    var role = null;
    await conn.query(`select User_DAP__r.Id,Role_Id__r.Name from Role_Right__c where User_DAP__r.Id = '${userId}'`, (err, result) => {
        if (err) console.log(err)
