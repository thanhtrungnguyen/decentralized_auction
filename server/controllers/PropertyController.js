...
// get list property by status
 const getListByStatus = async (req, res, next) => {
    try {
        ...
        await conn.query(`Select Id, Name,Status__c, Category_Id__r.Name,Start_Bid__c from Property_DAP__c where User_Id__c = '${userId}' And Status__c = '${req.params.status}'`, (err, result) => {
            if (err) console.error(err)
            properties = result.records
        })
        console.log(properties)
